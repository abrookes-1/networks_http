...
int processConnection(int sockFd) {
  ...
  while (keepGoing) {
    ...
    if (buffer[0] != '\0'){
      ...
      if (strlen(buffer) > 4 && strncmp(buffer, "CLOSE", 5) == 0){
        std::cout << "Connection closed by client." << std::endl;
        close(sockFd);
        keepGoing = 0;
      }
      else if (strlen(buffer) > 3 && strncmp(buffer, "QUIT", 4) == 0){
        std::cout << "Server shutdown by client." << std::endl;
        close(sockFd);
        keepGoing=0;
        quitProgram=1;
      }
...
int main (int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc,argv,"v")) != -1) {
    switch (opt) {
    case 'v':
      std::cout << "Verbose mode selected\n";
  ...
  while (bindSuccesful<0) {
    ...
    bindSuccesful = bind(listenFd, (struct sockaddr *)&servaddr, sizeof(servaddr));
  }
  DEBUG << "Socket bound" << std::endl;
  ...
  while (!quitProgram) {
    ...
    DEBUG << "Waiting for connection..." << std::endl;
